...
- [images/materials/colors.json](images/materials/colors.json) — 色票真相來源（86 色），`Number` ↔ `image` (`NN.webp`) ↔ `color` 必須對齊；改動會直接影響 swatch、預覽、工廠表格。
...
- [images/競速鞋參考圖片/](images/競速鞋參考圖片/) 與 [images/速樁鞋參考圖片/](images/速樁鞋參考圖片/) — 工廠選色單底圖 JPG（中文資料夾名稱是刻意的，請保留）。

### State model
單一 `state` 物件（[app.js:90-105](app.js#L90)）：
- `model`: `"speed"` 競速鞋（A/B/C 三區）或 `"slalom"` 速樁鞋（A/B 兩區，C 留在 state 但隱藏）。
- `activeZone`: 目前選擇中的分區。
...
- Slalom 用 2 區（A/B），有固定鞋口邊條 + A 區腳跟修正 mask；`mount` 欄位被鎖成 `165` 且 disabled（見 `updateMountFieldState`）。
